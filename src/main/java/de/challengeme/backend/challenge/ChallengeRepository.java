...
public interface ChallengeRepository extends JpaRepository<Challenge, Long> {
	...
	@Query(value = "SELECT * FROM challenges WHERE deleted = false AND category = :category AND id NOT IN(SELECT challenge_id FROM challengeresults WHERE user_id=:userId) ORDER BY rand() LIMIT 1", nativeQuery = true)
	public Challenge getRandomChallenge(String category, long userId);

	@Query(value = "SELECT * FROM challenges WHERE deleted = false AND category = :category ORDER BY rand() LIMIT 1", nativeQuery = true)
	public Challenge getRandomChallenge(String category);

	@Query(value = "SELECT * FROM challenges WHERE deleted = false ORDER BY rand() LIMIT 1", nativeQuery = true)
